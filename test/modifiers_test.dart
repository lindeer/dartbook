...
void main() {
  final entries = [
    GlossaryItem(
      'Henry (VII)',
      desc: '<p>Henry (VII) (1211 – 12? February 1242), a member of the '
          '<a href="https://en.wikipedia.org/wiki/Hohenstaufen">Hohenstaufen</a> dynasty</p>',
    ),
    GlossaryItem(
      'Conrad IV',
      desc: '<p>Conrad IV of Germany(25 April 1228 – 21 May 1254)</p>',
    ),
  ];
  ...
  test('annotate ignore class', () {
    final doc = parse(
      '<p class="no-glossary">Conrad IV is a brother of Henry (VII)</p>',
    );
    GlossaryModifier(entries).annotate(doc);
    expect(doc.querySelectorAll('.glossary-item').length, 0);
  });
  ...
  test('annotate ignore class', () {
    final doc = parse(
      'People said <code>Conrad IV</code> is a brother of Henry (VII)',
    );
    GlossaryModifier(entries).annotate(doc);
  ...
  test('annotate ignore class II', () {
    final doc = parse(
      'People said <span class="glossary-item">Conrad IV</span> is a brother of Henry (VII)',
    );
    GlossaryModifier(entries).annotate(doc);
  ...
  test('annotate ignore class III', () {
    ...
    final doc = parse(
      'People said <span class="glossary-item">Conrad IV</span> was Henry IV',
    );
    GlossaryModifier(items).annotate(doc);
    expect(doc.querySelectorAll('.glossary-item').length, 2);
  });
